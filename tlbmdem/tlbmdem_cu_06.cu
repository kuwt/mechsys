...
int main(int argc, char **argv) try
{
    ...
    dat.nu = nu;
    double rho = 1.0;
    dat.acc = Vec3_t(0.0,0.0,-1.0e-4);
    int id = 1;
    dom.DEMDOM.AddSphere(id,Vec3_t(0.5*dx*nx + R + 2*dx,0.5*dx*ny,0.5*dx*nz),R,rho);
    ...
    dom.PeriodicZ= true;
    double Tf = 1.0e5;
    dom.Solve(Tf,Tf/200,Setup,Report,"tlbmdem_cu_06",true,Nproc);
}
MECHSYS_CATCH
