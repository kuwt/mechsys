...
int main(int argc, char **argv) try
{
    ...
    if (!Parameter.isPeriodic){
        dom.DEMDOM.AddPlane (-1, Vec3_t(0.5*lx,0.5*ly,0), spongeSize, lx,ly, rho, M_PI, &axis0);
   ...
   // create particle position
    ...
    if (Parameter.ParticlePlacementMethod == 0) {
        pos = generate_particle_positions_lattice( Parameter.ParticleLatticeLayerNumber,
                                            ...
                                            Parameter.ParticleDist + 2 *Parameter.ParticleRadius + spongeSize,
    ...
    for (size_t k = 0;k<pos.size();k++) {
        ...
        std::mt19937 gen(rd());
        std::uniform_real_distribution<> distrib(0.0, 2 * M_PI);
        double angle = distrib(gen);
        double phi = distrib(gen);
        std::uniform_real_distribution<> distrib_cos_theta(-1.0, 1.0);
        double cos_theta = distrib_cos_theta(gen);
        double sin_theta = std::sqrt(1.0 - cos_theta * cos_theta);     
        Vec3_t ax(sin_theta * std::cos(phi),sin_theta * std::sin(phi),cos_theta);

        //dom.DEMDOM.AddPlane(id, pos[k], spongeSize, 2* R , R,rho, angle, &ax);
        //dom.DEMDOM.AddCube(id, pos[k], spongeSize, 2* R ,rho, angle, &ax);
        dom.DEMDOM.AddRecBox(id, pos[k], Vec3_t(2* R,2*R,R*0.5 ), spongeSize,rho,angle,&ax);
