...
int main(int argc, char **argv) try
{
    ...
    if (!Parameter.isPeriodic){
        dom.DEMDOM.AddPlane (BoxInitialTag-4, Vec3_t(0.5*dx*nx,0.5*dx*ny,0), R* 0.2, dx*nx,dx*ny, rho, M_PI, &axis0);
    ...
    dom.Solve(Tf,Tf/Parameter.OutputStep,Setup,Report,"tlbmdem_cu_11",true,Nproc);
}
MECHSYS_CATCH
